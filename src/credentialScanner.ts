...
export async function runCredentialScan(
  ...
  const uniqueFindings: CredentialThreat[] = [];
...
export function exportCredentialTsv(report: CredentialScanReport): string {
  ...
  for (const f of report.findings) {
    rows.push(
      `${f.locationType}\t${f.commit || f.branch || 'disk'}\t${f.file}${f.line ? `:${f.line}` : ''}\t${f.id}\t${f.redactedValue}\t${f.fingerprint}\t${f.description}`
    );
  }
  return rows.join('\n');
}
...
export function exportCredentialMarkdown(report: CredentialScanReport): string {
  ...
  for (const f of report.findings) {
    ...
    lines.push(
      `| ${sevBadge} | ${f.ruleName} | ${f.locationType} | \`${f.file}${f.line ? `:${f.line}` : ''}\` | \`${f.redactedValue}\` | \`${f.fingerprint}\` |`
    );
  }
