...
export interface CompromisedExtensionInfo {
  campaign: string;
  source: string;
}

export const COMPROMISED_EXTENSIONS: Record<string, CompromisedExtensionInfo> = {
...
export const WAVE_MARKER = ['lzcd', 'rtfx', 'yqip', 'lpd'].join('');

export const VS_RUN_MIN = 8;

export const INVISIBLE_UNICODE_RUN_REGEX = /[\uFE00-\uFE0F\u{E0100}-\u{E01EF}]{8,}/u;

export const INVISIBLE_UNICODE_BYTE_REGEX = /(?:\xEF\xB8[\x80-\x8F]|\xF3\xA0[\x84-\x87][\x80-\xBF]){8,}/;

export const LEGIT_LIB_REGEX = /(^|[\/])(pdf\.js|pdfjs|pdf\.worker|fontkit|opentype|harfbuzz|icu4x|unicode|grapheme|emoji-regex|twemoji|guardian-virus-scan|auditData|scanner)/i;

export const DECODER_PATTERN_REGEX = /(?:codePointAt\s*\([^)]*\)[\s\S]{0,500}(?:0xFE00|0xE0100|0xfe00|0xe0100)|(?:0xFE00|0xE0100|0xfe00|0xe0100)[\s\S]{0,500}codePointAt)/i;
