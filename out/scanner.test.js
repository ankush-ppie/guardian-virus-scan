...
async function testCredentialScanner() {
    ...
    assert.strictEqual((0, credentialScanner_1.isLowEntropy)('AIzaAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'), true, 'Repeated Google API key should be low entropy');
    assert.strictEqual((0, credentialScanner_1.isLowEntropy)('AIzaSyD-1234567890abcdefghijklmnopqrs'), false, 'Google API key with hyphens must pass entropy check');
    assert.strictEqual((0, credentialScanner_1.isLowEntropy)('AIzaSyDx_1234567890abcdefghijklmnopqr'), false, 'Google API key with underscores must pass entropy check');
    ...
    try {
        ...
        fs.writeFileSync(path.join(deepDir, 'strings.xml'), '<resources><string name="google_maps_key">AIzaSyDx1234567890abcdefghijklmnopqrstu</string></resources>\n');
        // Deep Flutter Dart file
        const dartDir = path.join(root, 'app', 'lib', 'features', 'maps', 'screens');
        fs.mkdirSync(dartDir, { recursive: true });
        fs.writeFileSync(path.join(dartDir, 'map_screen.dart'), 'const String mapboxToken = "pk.eyJ1234567890abcdefghijklmnopqrstuvwxyz01234.abcdefghijklmnopqrstuvwxyz";\n');
        ...
        (0, child_process_1.execFileSync)('git', ['add', 'shared_cred.js'], { cwd: root });
        (0, child_process_1.execFileSync)('git', ['commit', '--no-gpg-sign', '-m', 'Add shared aws key'], { cwd: root });
        const multiSourceReport = await (0, credentialScanner_1.runCredentialScan)(root, {
            scanTracked: true,
            scanHistory: true,
            scanLocal: true,
            scanRemotes: false,
        ...
        const getCredBanner = (h) => {
            const m = h.match(/<div class="summary-hero [^"]*" id="cred-summary-banner">([\s\S]*?)<\/div>/);
            return m ? m[1] : '';
        };
        const initialHtml = (0, report_1.buildReportHtml)(scanResultWithoutCreds, 'credential-scan');
        ...
        const vm = require('vm');
        const validateScript = (html, label) => {
        ...
        validateScript((0, report_1.buildReportHtml)(scanResultWithCreds, 'glassworm'), 'glassworm tab');
        validateScript((0, report_1.buildReportHtml)(scanResultWithCreds, 'extension-audit'), 'extension-audit tab');
    }
    finally {
        fs.rmSync(root, { recursive: true, force: true });
    }
